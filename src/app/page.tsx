...
function makeSnake(isPlayer: boolean, shape?: SnakeShape, paletteIdx?: number): Snake {
...
export default function Home() {
  ...
  // Scrollable skin list — all 18 skins stacked, selected one waves
  useEffect(() => {
    ...
    let scrollY = skinIdx * ROW_H - ch / 2 + ROW_H / 2;
    ...
    const drawSnakeRow = (segs: { x: number; y: number; a: number }[], skin: typeof SNAKE_PALETTES[0], shape: SnakeShape, animFrame: number, isAnimated: boolean, alpha: number) => {
      ...
      if (segs.length > 0) {
        ...
        const lookAngle = Math.atan2(-headWaveSlope, -SPACING);
        const normA = (a: number) => { while (a > Math.PI) a -= TAU2; while (a < -Math.PI) a += TAU2; return a; };
        const turnDelta = normA(lookAngle - headAngle);
        const eyeShift = Math.max(-1, Math.min(1, turnDelta / 0.5));
        const eo = BODY_R * 0.55, eyeSize = BODY_R * 0.38, pupilSize = eyeSize * 0.5;
        const pupilOffset = eyeSize * 0.4 * eyeShift;
        for (const side of [-0.7, 0.7]) {
          const ea = headAngle + side;
          const ex = hx + Math.cos(ea) * eo, ey = hy + Math.sin(ea) * eo;
          ctx.beginPath(); ctx.arc(ex, ey, eyeSize, 0, TAU2);
          ctx.fillStyle = '#fff'; ctx.fill();
          ctx.beginPath(); ctx.arc(
            ex + Math.cos(lookAngle) * pupilOffset + Math.cos(headAngle) * eyeSize * 0.12,
            ey + Math.sin(lookAngle) * pupilOffset + Math.sin(headAngle) * eyeSize * 0.12,
            pupilSize, 0, TAU2,
          );
    ...
    const onMove = (e: PointerEvent) => {
      ...
      scrollY = Math.max(0, Math.min((TOTAL - 1) * ROW_H, dragStartScroll - dy));
    ...
    const onWheel = (e: WheelEvent) => {
      e.preventDefault();
      const scale = cvs.height / (cvs.clientHeight || 1);
      scrollY = Math.max(0, Math.min((TOTAL - 1) * ROW_H, scrollY + e.deltaY * scale));
    };
    ...
    const draw = () => {
      ...
      if (!dragging) {
        targetScrollY = skinIdx * ROW_H - ch / 2 + ROW_H / 2;
      ...
      for (let idx = firstRow; idx <= lastRow; idx++) {
        ...
        // Selected indicator — dot + left accent
        if (isSelected) {
          ctx.fillStyle = skin.color;
  ...
  }, [screen, shapeIdx, skinIdx, previewShape, showSkinPicker]);
